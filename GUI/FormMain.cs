...
namespace GUI
{
    public partial class FormMain : Form
    {
        ...
        {
            TaiKhoanDAL taiKhoanDAL = new TaiKhoanDAL(_neo4jDriver); // Đảm bảo _neo4jDriver đã được khởi tạo

            // Lấy thông tin tài khoản
            TaiKhoan taiKhoan = await taiKhoanDAL.GetTaiKhoanInfo(this.MaNV);

            if (taiKhoan != null)
            {
                GDTaiKhoan formGD = new GDTaiKhoan(taiKhoan, taiKhoanDAL, this); // Truyền tham chiếu đến FormMain
                formGD.Show();
                this.Hide();
            }
            else
